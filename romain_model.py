...
def gaussian_sample(mean, var, scope=None):
    ...
    """
    print("This time with seed fixed")
    with tf.variable_scope(scope, 'gaussian_sample'):
        #sample = tf.random_normal(tf.shape(mean), 0.0, 1.0, seed=0)
        # b, n = 128, 10
        # answer = tf.convert_to_tensor(np.random.normal(0, 1, size=(b, n)), dtype=np.float32)
        #sample.set_shape(mean.get_shape())
        #return sample*tf.sqrt(var) + mean#answer#sample
        return tf.random_normal(tf.shape(mean), mean, tf.sqrt(var), seed=0)
...
class scVIModel:
    def __init__(self, expression=None, batch_ind=None, num_batches=None, kl_scale=None, mmd_scale=None, phase=None, \
        ...
        print(str(self.n_layers) + " hidden layers at " + str(self.n_hidden) + " each for a final " + str(
            self.n_latent) + " latent space")
    ...
    def generative_model(self):
        ...
        """
        print("Back with the none dropout")
