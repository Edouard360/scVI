...
def train_model(model, expression, sess, num_epochs, step=None, batch=None, kl=None):
    ...
    batch_size = 128
    iterep = int((expression_train.shape[0]-1) / float(batch_size)) + 1
    ...
    for t in range(iterep * num_epochs):
        ...
        #all_grads = model.optimize_algo.compute_gradients(model.loss, tf.trainable_variables())
        # run an optimization set
        #_, = model.train_step
        #all_grads, l_tr, qz_m, qz_v, z_, library_ = sess.run([all_grads, model.loss, model.qz_m,model.qz_v,model.z, model.library], feed_dict=dic_train)
        _ = sess.run([model.train_step], feed_dict=dic_train)
...
def eval_library(model, data, sess):
    dic_full = {model.expression: data, model.training_phase: False, model.kl_scale: 1}
    return sess.run(model.library, feed_dict=dic_full)

def eval_qz_v(model, data, sess):
    dic_full = {model.expression: data, model.training_phase: False, model.kl_scale: 1}
    return sess.run(model.qz_v, feed_dict=dic_full)
