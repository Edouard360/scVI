...
class PurePBMC(GeneExpressionDataset):
    def __init__(self, use_separate_batches=False):
        ...
        gene_names = pd.read_csv("data/mp/gene_names.csv", index_col=0).values.astype(np.str).ravel()
        gene_symbols = pd.read_csv("data/mp/gene_symbols.csv", index_col=0).values.astype(np.str).ravel()
